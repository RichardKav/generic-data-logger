...
public class UdpSender extends Sender {
    ...
    private void send(byte[] buffer, int len) throws IOException {
        for (InetSocketAddress address : _servers) {
            ...
            if (address.getAddress().isMulticastAddress()) {
                ...
                System.out.println("getSocket SEND");
    ...
    /**
     * Test of this sending data values mechanism
     * @param args 
     */
    ...
    public static void test() {
        System.out.println("Sending RK Test Values");
        UdpSender sender = new UdpSender();
        sender.addServer("localhost:25827");
        Identifier.Builder builder = Identifier.Builder.builder(); //Plugin type 0x0004 ??
        Identifier identifier = Identifier.Builder.builder().host("VM10-10-1-101").plugin("RKTEST").type("RKTEST").build();
//        Identifier identifier = Identifier.Builder.builder().plugin("RKTEST").pluginInstance("RKTestInstance").type("RKTest").build();
        Values value = new Values(identifier);
        value.addValue(111999111);
        try {
